...
public class MyApp {
    ...
    public static MyAppOberver observer;
    ...
    public void init(MyAppOberver obs, String app_dir)
    {
    ...
    public void init(MyAppOberver obs, String app_dir,
		...
		try {
			...
			//ep.libDestroy();
			Log.e("will", "ep.state:" + ep.libGetState());
			MyApp.observer.notifyEpState(ep);
		} catch (Exception e) {
			return;
		}
	...
	//	stun_servers.add("stun.pjsip.org");
		stun_servers.add("stun.fwdnet.net:3478");
    ...
    public MyAccount addAcc(AccountConfig cfg) {
		...
		MyAccount acc = MyAccount.getMyAccount();
		try {
			acc.create(cfg);
		} catch (Exception e) {
			acc = null;
