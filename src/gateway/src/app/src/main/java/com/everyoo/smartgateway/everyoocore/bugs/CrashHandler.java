...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    public void uncaughtException(Thread thread, Throwable ex) {
        if (ex == null && mDefaultHandler != null) {
            ...
            LogUtil.println(TAG + " uncaughtException","ex is unNull");
            handleException(ex);
        }
    }
    ...
    private void handleException(Throwable ex) {
        ...
        // 使用 Toast 来显示异常信息
        LogUtil.println(TAG + "handleException","thread id = " + Looper.myLooper().getThread().getId());
        LogUtil.println(TAG + "handleException","begin close dongle");
        new Thread() {
            @Override
            public void run() {
                 RestartAppUtil.restartApp(mContext, 0);//测试发现，程序如果出现异常，进程和服务都正常，但是无法控制，但是如果上报设备状态进程就会重启
            }
        }.start();
    }
